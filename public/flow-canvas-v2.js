...
function PromptNode({ data }) { return html`<div className="node prompt"><${Handle} type="source" position=${Position.Right} id="text" className="h-text" /><div className="head">Prompt</div><div className="body">${data.text || 'Double-click to edit in sidebar'}</div></div>`; }
function ImageNode({ data }) { return html`<div className="node image"><${Handle} type="source" position=${Position.Right} id="image" className="h-image" /><div className="head">Image</div><div className="body">${data.filename || 'No image'} </div></div>`; }
function WorkflowNode({ data }) { return html`<div className="node workflow"><${Handle} type="target" position=${Position.Left} id="prompt" style=${{top:16}} className="h-text" /><${Handle} type="target" position=${Position.Left} id="image" style=${{top:36}} className="h-image" /><${Handle} type="source" position=${Position.Right} id="workflow" className="h-workflow" /><div className="head">Workflow</div><div className="body">${data.templateName || 'Select template'}</div></div>`; }
function GenerateNode({ data }) { return html`<div className="node generate"><${Handle} type="target" position=${Position.Left} id="workflow" className="h-workflow" /><div className="head">Generate</div><div className="body">${data.status || 'Ready'}</div></div>`; }
...
function InnerApp() {
  ...
  useEffect(() => { (async()=>{
    try { const r = await fetch('/api/templates'); const j = await r.json(); setTemplates(j.ok ? (j.data||[]) : (Array.isArray(j)?j:[])); } catch {}
  })(); }, []);
  ...
  const addNode = useCallback((type) => {
    const c = rf.screenToFlowPosition({x:220+Math.random()*320,y:180+Math.random()*220});
    const n = { id: uid(type[0]), type, position:c, data:{} };
  ...
  const runGenerate = useCallback(async()=>{
    const gen = nodes.find(n=>n.type==='generate'); if (!gen) return;
    const edgeWG = edges.find(e=>e.target===gen.id && e.targetHandle==='workflow'); if (!edgeWG) { setLog('Generate not connected to workflow'); return; }
    ...
    try {
      ...
      const j = await r.json(); const data = j.ok ? j.data : j;
  ...
  return html`<div className="page">
    <div className="toolbar">
      ...
      <button className="btn" onClick=${runGenerate}>Run Generate</button>
    ...
    <div className="layout">
      ...
      <div className="sidebar">
        ...
        ` : html`<div className="muted">Select a node</div>`}
